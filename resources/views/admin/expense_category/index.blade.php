...
<div class="page-header d-flex align-items-start justify-content-between flex-wrap gap-3">
    <div><h1 class="page-title">{{ __('messages.expense_categories') }}</h1></div>
    @can('expense-category-add')
    <button type="button" class="btn-primary-sm" data-bs-toggle="modal" data-bs-target="#addECModal"><i class="bi bi-plus-lg"></i> {{ __('messages.add_new') }}</button>
    @endcan
</div>
    ...
    <div class="panel-card-body p-0">
        <div class="table-responsive">
            <table class="data-table">
                <thead><tr><th>#</th><th>{{ __('messages.field_name') }}</th><th>{{ __('messages.expenses') }}</th><th>{{ __('messages.Actions') }}</th></tr></thead>
                <tbody>
                    @forelse($categories as $c)
                    <tr>
                        <td>{{ $loop->iteration }}</td>
                        <td class="fw-semibold">{{ $c->name }}</td>
                        <td><span class="pill pill-info">{{ $c->expenses_count }}</span></td>
                        <td>
                            <div class="d-flex gap-1">
                                @can('expense-category-edit')
                                <button type="button" class="btn-icon-sm btn-edit" data-bs-toggle="modal" data-bs-target="#editECModal{{ $c->id }}"><i class="bi bi-pencil"></i></button>
                                @endcan
                                @can('expense-category-delete')
                                <form action="{{ route('admin.expense-category.destroy', $c->id) }}" method="POST" onsubmit="return confirm('{{ __('messages.confirm_delete') }}')">
                                    @csrf @method('DELETE')
...
<div class="modal fade" id="addECModal" tabindex="-1">
    <div class="modal-dialog">
        <form action="{{ route('admin.expense-category.store') }}" method="POST" class="modal-content">
            @csrf
